...
class Disasm:
    ...
    def __disasmble_functions(self) -> None:
        ...
        for _ in range(function_count):
            ...
            name = self.constants_table[name_index][1:-1]
    ...
    def __disasmble_op_codes(self) -> None:
        ...
        for _ in range(op_code_count):
            op_code = self.__read_bytes(Constants.OP_CODE_SIZE_FORMAT, 3)
            if OpCodeType(op_code[0]) == OpCodeType.PUSH:
                self.fp.write(f"    PUSH {str(self.constants_table[op_code[1]])}\n")
            else:
                self.fp.write(f"    {OpCodeType(op_code[0]).name}\n")
    ...
    def __load_constants_table(self) -> None:
        ...
        for _ in range(constants_count[0]):
            ...
            if data_type == DataType.INTEGER.value:
                ...
                assert False, "Unhandled DataType: " + data_type
